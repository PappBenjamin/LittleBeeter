...
#define EDGE_THRESHOLD 400   // Value that indicates ring edge - adjust based on your QTR sensor readings
...
RobotState currentState; // default state
...
// Function to print important information for debugging
...
{                    
  ...
  while(starter)
  {
      ...
      //print
      printImportantInfo(sensorValues, opponentDetected);
    ...
    switch(currentState)
    {
      ...
      case SEARCH:
      {
        ...
        if (sensorValues[3] < EDGE_THRESHOLD || sensorValues[3] < EDGE_THRESHOLD ||
            sensorValues[3] < EDGE_THRESHOLD || sensorValues[3] < EDGE_THRESHOLD)
        {
      ...
      case ATTACK:
      {
        ...
        {
          // Edge detected, retreat (not STOP)
          currentState = RETREAT;
          stateTimer = millis() + 500; // Retreat for 500ms
    ...
    starter = digitalRead(startPin); // Check start button again

    delay(2000);

  }

  stopMotors(); // Stop motors if not in any state
