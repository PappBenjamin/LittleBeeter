...
void loop() {
  ...
  // Read IR sensor for opponent detection
  int opponentDetected = digitalRead(IRPins[0]); // Assuming HIGH when opponent detected
  ...
  switch (currentState) {
    ...
    case SEARCH:
      ...
      if (sensorValues[0] < EDGE_THRESHOLD) {
    ...
    case ATTACK:
      ...
      if (sensorValues[0] < EDGE_THRESHOLD || sensorValues[1] < EDGE_THRESHOLD || sensorValues[2] < EDGE_THRESHOLD || sensorValues[3] < EDGE_THRESHOLD) {
        // Edge detected, retreat
        currentState = STOP;
        stateTimer = millis() + 500; // Retreat for 500ms
